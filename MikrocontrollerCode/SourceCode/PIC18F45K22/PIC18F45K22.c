    ...
    #define StepYL LATC.B2   //  Step for 2 LEFT Y motors
        ...
        //  Z STEPPER
     // Place jumper for testing and laser Z carriage without Z motor
...
void main()
    ...
    Delay_ms(200);          //  Wait for UART module to stabilize
     ...
     CommandIndex=0;
            ...
            // TIMER0  +-100 usec
            // line below is only for PIC18F45K22
  ...
  // Escape mode in case of X or Y cornerswitch is closed, this means moving without checking cornerswitch
   ...
   void  ST28( unsigned int Direction,unsigned int Steps) //ST 28 unipolar motor
       ...
       void Initialisation(void)
              ...
              //  Z AXIS HOME :
               ...
               //  enough escape moves topwards so that BOTH Y switches are open again.
                ...
                // old code also for Z99PP
              /*

    # include <built_in.h>

    #define EscapeSteps 600  //  switch level has some hysteresis
                             // 600 microsteps =  3.0 mm
    ...
    #define StepYL LATC.B2   //  Step for 2 LEFT Y motors
        ...
        //  Z STEPPER
     // if jumper is placed = JumperZ is low = no Z motor = for laser Z carriage without Z motor
     // if jumper is NOT placed = JumperZ is high = Z motor for all other Z carriages.
    ...
    #define StepZ LATA.B5       //  pin 7
      ...
      //  R ROTATE MOTOR
    #define DirR  LATE.B2      //  F2 Rotate direction    pin 10
    #define StepR LATE.B1      //  F1 Rotate step         pin 9

    // SUCTION:
    
     #define SUC1  PORTA.B3    // MPX analog value

     // CORNER SWITCHES
     // REMEMBER CLOSED SWITCH = 0
     ...
     #define Relais1 LATC.F5   //
     #define Relais2 LATC.F4  //
     #define Relais3 LATD.F3   //
    ...
    //  unipolar stepper is only used for ST28 dispenser stepper
      ...
      unsigned int AdVal=0,AdVal1,AdVal2,AdVal3,AdVal4,AdVal5,AdVal6,AdVal7,AdVal8;
    ...
    // always downwards
     ...
     void Rotate(unsigned int,unsigned int);
     void ST28(unsigned int,unsigned int);

      // ADDED 12 04 2020
     void ZSpeed(unsigned int,unsigned int);
     void RSpeed(unsigned int,unsigned int);
     
     int Analog_Value(int);
     void Initialisation(void);
        ...
        int XYMotorDelay =25;     // Speed of XY steps. See function XYSlowDown for  X and Y steps/sec.
        int ZMotorDelay= 75;     // Speed of ZR steps. See function ZRSlowDown for  Z and R steps/sec.
        int RMotorDelay=150; // = slower
...
void main()
       ...
       int x=0;
       int y=0;
      unsigned int StepsX=0;  // u
      unsigned int StepsY=0;  // u
      unsigned int StepsZ=0;    // u
      unsigned int StepsR=0;      // u
      unsigned int StepsT=0;    // tune bottom camera
    ...
    TRISA  = 0b00001110;      //   PORTA.A2 and A3 are input  for MPX5010DP suction
                              //   A0  = out to enable XY motors
                             //    A1 = DIGITAL INPUT FOR Z JUMPER
                             // no jumper = high = default operation with Z motor
                             // jumper placed = low = for operation without Z motor, laser module

    TRISB = 0b11111111;      // was 11111100 1 = set direction B0..B5 to be input !
   ...
   //   LATB = 0x00;              // Corner switches
    ...
    Delay_ms(200);          //  Wait for UART module to stabilize
     ...
     ADC_Init();      //   not used by Z99
      

     CommandIndex=0;

     // before entering the loop position at origin
     // will be ok for plotter, drill and cutter, but not for
     //  pick and place. We need then optical control.
     ...
     //  BELOW THE CODE FOR XYZzRrMOTION
          ...
          Initialisation();      // Will home Z Y and X in this order
        // enable timer 0 interrupt
           TMR1IE_bit    =1;      // enable timer 1 interrupt
              ...
              if (UART1_Data_Ready())
               ...
               if ( TestCheckSum ==  CheckSum   )
                    ...
                    switch(Commands[0])
                      ...
                      case 68:   //   D   = MotorDelay, steps /sec of XY steppers
                                 //  OLD :   MotorDelay= Commands[1];
                      ...
                      case 70:   //   70=F Feed Component  , not executed in this PCB but in FEEDER PCB

                   //  PC SHOULD CALCULATE HOW MUCH TIME THIS TAKES AND WAIT LONG ENOUGH AFTER FEED COMPONENT INSTRUCTION !!!!
                             Uart1_Write(43);
                             CommandIndex = 0;
                             break;
                      ...
                      case 75: // L = laser
                               ...
                               case 0: LASER=0;          //= ON       Pin 21 of PIC D.F2 low
                                        break;
                               case 1: LASER=1;        // = OFF     Pin 21 of PIC D.F2 low
                                        break;
                              default: break;
                                        }
                                 Uart1_Write(43);
                      ...
                      case 77:  //  M = data from MPX5010DP
                                Uart1_Write( 112);
                                AdVal1 = Analog_Value(Commands[1]);
                               // BELOW OK, ADD LATER
                                delay_us(300);                             // MAYBE REMOVE DELAY!
                                AdVal2 = Analog_Value(Commands[1]);
                                delay_us(300);                                 // MAYBE REMOVE DELAY!
                                AdVal3 = Analog_Value(Commands[1]);
                                delay_us(300);                        // MAYBE REMOVE DELAY!
                                AdVal4 = Analog_Value(Commands[1]);
                                AdVal= (unsigned int)((AdVal1+AdVal2+AdVal3+AdVal4)/4);
                                         // delay_us(300);
                                 Uart1_Write( Hi(AdVal) );        // LSB
                                 Uart1_Write( Lo(AdVal) );
                                 Uart1_Write(43);
                                 CommandIndex=0;
                                break;
                     ...
                     case 82:         //  R = rotation
                            ...
                            StepsR = ( Commands[2]  << 8 ) + Commands[3];
                            Rotate((unsigned int) Commands[1], (unsigned int) StepsR );
                            Uart1_Write(43);
                            CommandIndex=0;
                            break;
                            
                 // added 12 04 2020 :
                     case 83: //  speed of Rotate motor, RMotorDelay;
                                RMotorDelay = (Commands[1] << 8 ) + Commands[2];
                                Uart1_Write(43);
                                CommandIndex=0;
                              break;

                     // This function serves for unipolar ST28 stepper
                     case 84:    /// T = tune
                            StepsT =  (Commands[2] << 8 ) + Commands[3];
                            ST28(Commands[1], StepsT);
                            Uart1_Write(43);
                           CommandIndex=0;
                             break;
                      case 85:  //   U  of  Suction channel  1 or 2
                            switch(Commands[1])
                                       {
                               case 1: Relais1=1; Relais2=0;  // EM (electromagnet) ON
                                        break;
                               case 2: Relais1=0; Relais2=1;  // SUC (suction) ON
                                        break;
                               case 3: Relais1=1; Relais2=1;  // EM and SUC  ON.
                                        break;
                               case 4: Relais1=0; Relais2=0;  // EM and SUC OFF
                                        break;
                               default: break;
                                        }
                                    Uart1_Write(43);
                     ...
                     case 86: // V Velocity, speed of Z and R motor, ZR MotorDelay;
                                ZMotorDelay = (Commands[1] << 8 ) + Commands[2];
                                Uart1_Write(43);
                                CommandIndex=0;
                              break;
                              
              //PART I  XY MOVEMENTS < 65536
              
                     case 87:         //  Dir 1    W  of  XY dir to left inferior corner
                            DirX=0;
                            DirYR = 0;
                            DirYL=DirYR;          // Direction is the same on both Y motors, one coil is crossed over on the PCB or connection to Y motor !
                            ...
                            CommandIndex=0;
                             break;
                    case 89:                   // DIR 3 dir to left above
                              DirX=0;
                              DirYR = 1;
                              DirYL=DirYR;          //
            ...
            //   PART IV   X or Y > 196605
                ...
                // START WITH Z
                      ...
                      case 111:
                       ...
                       case 113:
                            DirX=1;   // GO UPWARDS OR LEFTWARDS WITHOUT CHECKING CORNERSWITCH
                            DirYR = 1;
                            DirYL=DirYR;
                            EscapeMotorXY(Commands[1],Commands[2],Commands[3],Commands[4]);
                            Uart1_Write(43); // maybe not
                    ...
                    else     // if checksum was not ok
                    {
                                    Uart1_Write(63); // aks for same instruction once more
                                    CommandIndex=0;
                            // reset also the pic in case of checksum error
    ...
    void MotorXY(unsigned int xSteps,unsigned int ySteps, unsigned short parts)
              {
              unsigned int M=0;
              unsigned long stepperDelayX=0;     //  for stepper
              unsigned int timerStepperDelayX=0;
              unsigned long stepperDelayY=0;     //  for stepper
               unsigned int timerStepperDelayY=0;
              unsigned int counterStepX=0;
              unsigned int counterStepY=0;
              unsigned int z=0; // used for timer delay
           ...
           // PART IV
             ...
             // NEW
              ...
              // partOf1600/5;   /// WAS 10  20 slechter !! 5 beter

             stepperDelayX=XYMotorDelay*2*15;    // * 2 2*delay *15 from 14.7456MHZ clock count/sec
                  
                  // 2 5 2020 = Y , less most steps were done somewhat to quick, adding +200 = much much better
                  // HOWEVER 400 WAS TO MUCH !! +-1/5 Y steps lost
                  // After calculations of extra delay of 1600 first and last microsteps 280 was ok
                  // as extra delay for Y motors
             ...
             stepperDelayY=(unsigned int)(((long)stepperDelayX*(long)xSteps)/(long)ySteps); // +80 nearly ok
              ...
              // partOf1600/5;  // /// WAS 10  20 : slechter
                  ...
                  stepperDelayY=XYMotorDelay*2*15;    // * 2 2*delay *15 from 14.7456MHZ clock count/sec
                      ...
                      // 2 5 2020 = Y , less most steps were done somewhat to quick, adding +200 = very much better
                    // HOWEVER 400 WAS TO MUCH !! +-1/5 X steps lost
                    // 240 was to much +-3 cm y to less
         ...
         DirZ=0;     //  always downwards !
          ...
          StepZ=1;
              Delay_us(200);
                     ...
                     // not used by Z99:
   void Rotate( unsigned int Direction,unsigned int Steps)
       {
         int x;
         int Rsteps =0;
         // 2  NEW LINES:
          if (Direction==0) DirR=0;
          else DirR=1;
   ...
   void  ST28( unsigned int Direction,unsigned int Steps) //ST 28 unipolar motor
       ...
       void ZSpeed(unsigned int ZStepsDone,unsigned int ZStepsToDo)
       {
         int x;
              ...
              //  also some SlowDown on landing ;
              //  is maxmost     1.15 m sec
                   if (ZStepsDone<10)    Delay_us(300);
                   if (ZStepsToDo<20)    Delay_us(300);
                   if (ZStepsToDo<40)    Delay_us(250);
                   if (ZStepsToDo<80)    Delay_us(200);
      ...
      void RSpeed(unsigned int RStepsDone,unsigned int RStepsToDo)
       {
         int x;
                 // SlowDown on take of
                 // is maxmost 400+400+350+300+150 = 1.6 msec extra

                   if (RStepsDone<10)    Delay_us(400);
                   if (RStepsDone<20)    Delay_us(400);
                   if (RStepsDone<40)    Delay_us(350);
                   if (RStepsDone<80)    Delay_us(300);
                   if (RStepsDone<160)    Delay_us(150);

              //  also some SlowDown on landing ;
              //  is maxmost     1.15 m sec
                   if (RStepsDone<10)    Delay_us(300);
                   if (RStepsToDo<20)    Delay_us(300);
                   if (RStepsToDo<40)    Delay_us(250);
                   if (RStepsToDo<80)    Delay_us(200);
                   if (RStepsToDo<160)    Delay_us(100);

           for (x=0;x<RMotorDelay;x++) Delay_us(1);              // core
     }



        int Analog_Value(int Number)    // Suction value from MPX5010DP
           {
        return ( ADC_Read(Number) ) ;         // 10 bits value
           }
        ...
        //  Z AXIS HOME :
                ...
                if (JumperZ==1)   // jumper Z is not placed, Z motor is in use
                     ...
                     if (ZCORNERSWITCH==0) break;
                        ...
                        DirZ=0;     // downwards     20 mm default =8000 microsteps

                     for(x=0;x<2000;x++)   //2000=2.5 mm, previous 1200 = 1.5 mm
                                {
                                   Delay_us(100);   //  slow  maxmost  2000 steps/sec - 2000 /16 = 125  steps /sec->250 full steps /sec
                                   StepZ=0;         //
                                   Delay_us(100);   // slow maxmost 2000 steps/sec
                  ...
                  while(1)
                    ...
                    if (XLCORNERSWITCH==0) break;
                     ...
                     //  a full rotation = 200*16=3200 microsteps of pulley with GT2 and 20 teeths advances 40 mm.
                      ...
                      for(x=0;x<(EscapeSteps);x++)   // WAS 200
                                {
                                   Delay_us(50);   // slow
                                   StepX=0;         //
                                   Delay_us(50);   //  slow
