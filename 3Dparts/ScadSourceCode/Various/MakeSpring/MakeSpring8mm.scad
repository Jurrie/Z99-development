// Do NOT MIRROR IN SLIC3R  !
// ATTENTION !
// BE PATIENT, RENDERING (F6) WILL TAKE SOME TIME !
...
rotate([180,0,0])    // I'm right handed
mirror([0,0,180])    // Left handed people better omit this
                    // rotate and mirror instructions !!!!
...
{  
    translate([0,0,-30])       
  metric_thread (diameter=11.5, pitch=3.0, length=40);
    // 11.5 is ok for spring  mm with 8 mm internal diameter

/// foot makes printing without brim possible :
translate([0,0,-30.5]) 
cylinder(h=2,d=16,center=true,$fn=48);
    
    // Turn this foot clockwise ( seen from bottom up )
    // to free it from spring 
    // or to make longer spring
    
    
    
}

  translate([0,0,-30])  // 0 0 6
  cylinder(d=5.0,h=62,center=false,$fn=48); // 5.4
/*
translate([0,0,30]) 
cube([30,1,6],center=true);
*/
// comment out for demo sagittal section
/*
translate([-10,0,20])
cube([20,20,44],center=true);
*/
...
difference () {
   ...
   metric_thread (diameter=34, pitch=2, length=14, internal=true, n_starts=6);
 }
*/
// ----------------------------------------------------------------------------
